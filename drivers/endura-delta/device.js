...
class MyDevice extends Device {
  ...
  async onInit() {
    ...
    this._updateInterval = this.homey.setInterval(async () => {
    ...
    this.registerCapabilityListener("filter_days", async (value) => {
      try {
        ...
        for (const arg of args) {
          if (value === arg.days) {
            // Změnili jsme podmínku na přesnou shodu
    ...
    this._filterDaysTrigger = this.homey.flow.getDeviceTriggerCard(
      "filter_days_remaining"
    );
    this._lastFilterDays = null;

    // Register Flow Card Actions
    this.setVentilationTimerAction = this.homey.flow.getActionCard(
      "set_ventilation_timer"
    );
    ...
    this.stopVentilationTimerAction = this.homey.flow.getActionCard(
      "stop_ventilation_timer"
    );
    ...
    this.startBreezeModeAction =
      this.homey.flow.getActionCard("start_breeze_mode");
    ...
    this.setHolidayModeAction =
      this.homey.flow.getActionCard("set_holiday_mode");
    ...
    this.setAutomaticBreezeAction = this.homey.flow.getActionCard(
      "set_automatic_breeze"
    );
  ...
  async getProductionData() {
    try {
      ...
      await this.setCapabilityValue(
        "measure_power.current_level",
        parseInt(deviceCurrentVentilationLevel.split("Level")[1])
      );
      await this.setCapabilityValue(
        "measure_wind_strength.pulse_airflow",
        parseInt(deviceMeasuredSupAirflow)
      );
      await this.setCapabilityValue(
        "measure_wind_strength.eta_airflow",
        parseInt(deviceMeasuredEtaAirflow)
      );
      await this.setCapabilityValue(
        "measure_wind_strength.nominal_airflow",
        parseInt(deviceTotalNominalAirflow)
      );
      ...
      await this.setCapabilityValue(
        "frost_protection_active",
        deviceFrostProtectionActive === "1"
      );
      await this.setCapabilityValue(
        "preheater_active",
        devicePreheaterEnabled === "1"
      );
      await this.setCapabilityValue(
        "breeze_active",
        deviceBreezeConditionsMet === "1"
      );
  ...
  async onDeleted() {
    ...
    // Clean up interval on deletion
    if (this._updateInterval) {
      this.homey.clearInterval(this._updateInterval);
    }
  }
}
